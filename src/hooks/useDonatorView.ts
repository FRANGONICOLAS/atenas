...
import type { Donation, DonationStatus, DonationType, Project } from "@/types";
...
export const useDonatorView = () => {
  ...
  // Stats state
  const [stats, setStats] = useState<DonatorStats>({
    totalDonated: 0,
    projectsSupported: 0,
    lastDonation: "",
  });
  ...
  const handleCancelDonation = (donationId: number) => {
    setDonations((prev) =>
      prev.map((d) =>
        d.id === donationId
          ? { ...d, status: "cancelled" as DonationStatus }
          : d,
      ),
    );
  ...
  const handleSaveDonation = (donation: Partial<Donation>) => {
    if (donation.id) {
      setDonations((prev) =>
        prev.map((d) =>
          d.id === donation.id ? ({ ...d, ...donation } as Donation) : d,
        ),
      );
