...
const MainDashboard = ({ user }: { user: User }) => {
  ...
  if (loading) {
    return <FullScreenLoader message={t.donatorDashboard.status.loading} />;
  }
    ...
    return (
      <div className="w-full">
        ...
        <Alert variant="destructive">
          ...
          <AlertTitle>{t.donatorDashboard.status.error}</AlertTitle>
  ...
  const statsCards = [
    {
      icon: DollarSign,
      title: t.donatorDashboard.stats.totalDonated,
      value: formatCurrency(stats?.totalDonated || 0),
      color: "bg-green-500",
    },
    {
      icon: Trophy,
      title: t.donatorDashboard.stats.projectsSupported,
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t.donatorDashboard.titles.projects}
            </CardTitle>
            <CardDescription>
              {t.donatorDashboard.descriptions.contributionsStatus}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {stats?.supportedProjects && stats.supportedProjects.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-4">
                {t.donatorDashboard.status.noProjects}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t.donatorDashboard.titles.recentDonations}
            </CardTitle>
            <CardDescription>
              {t.donatorDashboard.descriptions.recentContributions}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {stats?.recentDonations && stats.recentDonations.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-4">
                {t.donatorDashboard.status.noRecent}
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t.donatorDashboard.titles.impact}
          </CardTitle>
          <CardDescription>
            {t.donatorDashboard.descriptions.directBenefits}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center space-y-2 p-4 rounded-lg bg-muted/50">
              ...
              <p className="text-sm text-muted-foreground">
                {t.donatorDashboard.stats.projectsSupported}
              </p>
            </div>
            <div className="text-center space-y-2 p-4 rounded-lg bg-muted/50">
              <p className="text-3xl font-bold text-primary">
              ...
              <p className="text-sm text-muted-foreground">
                {t.donatorDashboard.stats.donationsMade}
              </p>
            </div>
          </div>
        </CardContent>
