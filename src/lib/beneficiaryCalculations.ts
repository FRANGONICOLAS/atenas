...
const collectTechnicalScores = (
  technicalData?: TechnicalTacticalData | null,
): number[] => {
 ...
 */
export const extractSex = (
  antropometricData?: AntropometricData | null,
): string | undefined => {
 ...
 */
export const getTechnicalAverage = (
  technicalData?: TechnicalTacticalData | null,
): number => {
  const skills = collectTechnicalScores(technicalData);
 ...
 */
const extractFantasticScore = (
  emotionalData?: EmotionalData | null,
): number => {
  if (!emotionalData) return 0;

  const answers = Array.from({ length: 45 }, (_, index) => {
    const key = `fantastic_${index + 1}` as keyof EmotionalData;
    const rawValue = emotionalData[key];
    ...
    if (
      ...
      Number.isFinite(rawValue.value)
    ) {
...
export const getEvaluationScore = (evaluation: EvaluationRow): number => {
  switch (normalizeEvaluationType(evaluation.type)) {
    ...
    case "ANTHROPOMETRIC": {
      const data = getEvaluationDetailByType(
        evaluation,
      ) as AntropometricData | null;
      if (!data) return 0;
    ...
    case "EMOTIONAL":
      return extractFantasticScore(
        evaluation.emotional_detail as EmotionalData | null,
      );
    default:
      return 0;
  }
